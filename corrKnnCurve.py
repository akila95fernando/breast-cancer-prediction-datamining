...
# drop row with nan values (remvoe missing)
# initial_data.dropna(inplace=True)
# initial_data.fillna(initial_data.mean(), inplace=True)
missingCols = ['concavity_mean','concave points_mean','concavity_se','concave points_se','concavity_worst','concave points_worst']
initial_data[missingCols] = initial_data[missingCols].fillna(initial_data.mode().iloc[0])
# print((initial_data == 0).sum())
...
X_corr = preprocessed_data[['smoothness_mean', 'radius_se', 'texture_se', 'smoothness_se', 'symmetry_se',
                       'fractal_dimension_se', 'texture_worst', 'symmetry_worst','fractal_dimension_worst']]
Y_corr = preprocessed_data['diagnosis']

X_train, X_test, Y_train, Y_test = train_test_split(X_corr, Y_corr, test_size=0.34, random_state = 0)
...
for k in k_range:
    ...
    scores[k] = metrics.accuracy_score(Y_test, predictKNN)
    scores_list.append(metrics.accuracy_score(Y_test, predictKNN))
